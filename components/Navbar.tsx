...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="md:hidden bg-charcoal-light border-t border-gray-800 animate-fadeIn">
          <div className="px-4 pt-3 pb-6 space-y-1">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="block px-3 py-2.5 text-gray-300 hover:text-gold-500 hover:bg-gray-800/60 rounded-lg transition-colors text-sm font-medium"
            ...
            <div className="pt-3">
              <a
                ...
                className="flex items-center justify-center gap-2 w-full py-3 rounded-xl
                          bg-gold-500 text-black font-bold text-sm tracking-wide
